...
# 添加这些导入
from fast_models.rdt_runner_flow_matching import RDTRunner
...
os.environ['TF_NUM_INTEROP_THREADS'] = '4'  # 限制TensorFlow的并行线程数
...
class FinetuneConfig:
    ...
    use_rdt: bool = False                            # If True, trains continuous action head with RDT objective
    num_diffusion_steps_train: int = 50              # (When `diffusion==True`) Number of diffusion steps used for training
    use_film: bool = False                           # If True, uses FiLM to infuse language inputs into visual features
    num_images_in_input: int = 1                     # Number of images in the VLA input (default: 1)
    use_proprio: bool = False                         # If True, includes robot proprioceptive state in input

    # RDT specific parameters
    rdt_config_path: Optional[str] = None            # Path to RDT config YAML file
    pretrained_rdt_path: Optional[str] = None        # Path to pretrained RDT model (optional)

    # 添加SiglipVisionTower配置
    pretrained_vision_encoder_name_or_path: str = "google/siglip-so400m-patch14-224"  # SiglipVisionTower路径

    # Training mode control
    slow_train_mode: bool = True                          # If True, sets model to train mode; if False, sets to eval mode
    ...
    # Logging
    use_wandb: bool = False                           # If True, uses Weights & Biases for logging
...
def run_forward_pass(
    ...
    Args:
        ...
        siglip_vision_encoder (SiglipVisionTower): SiglipVisionTower for generating image tokens.
        batch (dict): Input batch.
        action_tokenizer (ActionTokenizer): Action tokenizer.
        device_id (str): Device ID.
        use_l1_regression (bool): Whether to use L1 regression.
        use_diffusion (bool): Whether to use diffusion.
        use_rdt (bool): Whether to use RDT.
    ...
    # print(f"ground_truth_actions (first sample):\n{ground_truth_actions[0].cpu().detach().float().numpy().round(2)}")
    ...
    if not (use_l1_regression or use_diffusion or use_rdt):
        loss = output.loss
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion | RDT)
    else:
        ...
        if use_rdt:
            ...
            # lang_tokens shape: (B, text_seq_len, D)
            batch_size = lang_tokens.shape[0]
            text_seq_len = lang_tokens.shape[1]
            lang_attn_mask = torch.ones(batch_size, text_seq_len, device=device_id, dtype=torch.bool)
            # print(f"Created full attention mask with shape: {lang_attn_mask.shape}")

            with torch.no_grad():
                channels_per_image = 6  # openvla uses 6 channels per image (dinov2+siglip)
                rgb_channels = 3
                pixel_values = batch["pixel_values"].to(torch.bfloat16).to(device_id)

                B, total_channels, H, W = pixel_values.shape
                # print(f"pixel_values shape: {pixel_values.shape}")  # (B, C, H, W)

                assert total_channels % channels_per_image == 0, \
                    f"Expected channels divisible by {channels_per_image}, got {total_channels}"

                num_images = total_channels // channels_per_image

                pixel_values = pixel_values.reshape(B, num_images, channels_per_image, H, W)
                rgb_images = pixel_values[:, :, rgb_channels:, :, :] 
                rgb_images = rgb_images.reshape(B * num_images, rgb_channels, H, W)
                # print(f"rgb_images shape: {rgb_images.shape}")
                # print(f"rgb_images content: {rgb_images}")

                img_tokens = siglip_vision_encoder(rgb_images).detach()
                # print(f"img_tokens type: {type(img_tokens)}")
                img_tokens = img_tokens.reshape(B, num_images * img_tokens.shape[1], -1)


            if use_proprio:
                state_tokens = batch["proprio"].unsqueeze(1)  # (B, 1, state_dim)

            action_mask = torch.ones(batch_size, 1, ACTION_DIM,
                                   device=lang_tokens.device,
                                   dtype=torch.bfloat16)

            ctrl_freqs = torch.ones(batch_size, device=lang_tokens.device) * 10.0

            if lang_attn_mask.dtype != torch.bool:
                lang_attn_mask = lang_attn_mask.bool()
            ...
            if print_input:
                ...
                print(f"lang_attn_mask shape: {lang_attn_mask.shape}")
                print(f"img_tokens shape: {img_tokens.shape}")
                print(f"state_tokens shape: {state_tokens.shape}")
                print(f"ground_truth_actions shape: {ground_truth_actions.shape}")
                print(f"action_mask shape: {action_mask.shape}")
                print(f"state_tokens: {state_tokens}")  # Debugging: Print the content of state_tokens
                print(f"ground_truth_actions: {ground_truth_actions}")  # Debugging: Prin

            loss = action_head.module.compute_loss(
                lang_tokens=lang_tokens,
                lang_attn_mask=lang_attn_mask,
                img_tokens=img_tokens,
                state_tokens=state_tokens,
                action_gt=ground_truth_actions,
        ...
        should_log_l1_loss = not use_diffusion or (use_diffusion and compute_diffusion_l1) or not use_rdt
...
def log_metrics_to_wandb(metrics, prefix, step, use_wandb_flag) -> None:
    """
    ...
    Args:
        ...
        use_wandb_flag (bool): Whether to use W&B logging

    Returns:
        None.
    """
    if not use_wandb_flag:
        return
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        if cfg.use_rdt:
            rdt_action_head_dir = checkpoint_dir / "rdt_action_head"
            os.makedirs(rdt_action_head_dir, exist_ok=True)
    ...
    if distributed_state.is_main_process:
        ...
        # Save action heads with different names based on type
        if cfg.use_rdt and action_head is not None:
            rdt_action_head_dir = checkpoint_dir / f"rdt_action_head_step_{log_step}"
            os.makedirs(rdt_action_head_dir, exist_ok=True)
            action_head.module.save_pretrained(rdt_action_head_dir)
            save_dataset_statistics(train_dataset.dataset_statistics, rdt_action_head_dir)
        elif (cfg.use_l1_regression or cfg.use_diffusion) and action_head is not None:
...
def run_validation(
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            ...
            _, metrics = run_forward_pass(
                ...
                noisy_action_projector=noisy_action_projector,
                proprio_projector=proprio_projector,
                siglip_vision_encoder=None,  # Validation does not use SiglipVisionTower
                batch=batch,
    ...
    if distributed_state.is_main_process:
        log_metrics_to_wandb(avg_val_metrics, "VLA Val", log_step, cfg.use_wandb)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    siglip_vision_encoder = SiglipVisionTower(
        vision_tower=cfg.pretrained_vision_encoder_name_or_path,
        args=None
    )
    siglip_vision_encoder.vision_tower.to(device_id, dtype=torch.bfloat16)
    ...
    # If applicable, instantiate RDT action head
    if cfg.use_rdt:
        with open(cfg.rdt_config_path, "r") as fp:
            rdt_config = yaml.safe_load(fp)
            print(f"Loaded RDT config from {cfg.rdt_config_path}")
        
        if (
            hasattr(cfg, 'pretrained_rdt_path') and
            cfg.pretrained_rdt_path is not None and
            not os.path.isfile(cfg.pretrained_rdt_path)
        ):
            print("Loading RDT from pretrained checkpoint.")
            rdt_runner = RDTRunner.from_pretrained(cfg.pretrained_rdt_path)
            action_head = rdt_runner
            action_head = action_head.to(device_id)
            if not isinstance(action_head, DDP):
                action_head = wrap_ddp(action_head, device_id, find_unused=True)
        else:
            print("Constructing RDT model from provided config.")
            # Calculate the image condition length
            img_cond_len = (rdt_config["common"]["img_history_size"] 
                            * rdt_config["common"]["num_cameras"] 
                            * siglip_vision_encoder.num_patches)

            weight_dtype = torch.bfloat16
            
            rdt = RDTRunner(
                action_dim=rdt_config["common"]["state_dim"],
                pred_horizon=rdt_config["common"]["action_chunk_size"],
                config=rdt_config["model"],
                lang_token_dim=rdt_config["model"]["lang_token_dim"],
                img_token_dim=rdt_config["model"]["img_token_dim"],
                state_token_dim=rdt_config["model"]["state_token_dim"],
                max_lang_cond_len=rdt_config["dataset"]["tokenizer_max_length"],
                img_cond_len=img_cond_len,
                img_pos_embed_config=[
                    # No initial pos embed in the last grid size
                    # since we've already done in ViT
                    ("image", (rdt_config["common"]["img_history_size"], 
                        rdt_config["common"]["num_cameras"], 
                        -siglip_vision_encoder.num_patches)),  
                ],
                lang_pos_embed_config=[
                    # Similarly, no initial pos embed for language
                    ("lang", -rdt_config["dataset"]["tokenizer_max_length"]),
                ],
                dtype=weight_dtype,
            )
    ...
    if cfg.use_l1_regression or cfg.use_diffusion or cfg.use_rdt:
        trainable_params += [param for param in action_head.parameters() if param.requires_grad]
        print(f"# trainable params in action head: {sum(p.numel() for p in action_head.parameters() if p.requires_grad)}")
    if cfg.use_diffusion:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss, metrics = run_forward_pass(
                ...
                siglip_vision_encoder=siglip_vision_encoder,
                batch=batch,
                action_tokenizer=action_tokenizer,
                device_id=device_id,
                use_l1_regression=cfg.use_l1_regression,
            ...
            if distributed_state.is_main_process and log_step % cfg.wandb_log_freq == 0:
                log_metrics_to_wandb(smoothened_metrics, "VLA Train", log_step, cfg.use_wandb)
            ...
            if (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                ...
                current_loss = metrics.get("loss_value", 0.0)
            ...
            if gradient_step_idx > 0 and log_step % cfg.save_freq == 0:
                save_training_checkpoint(
                    ...
                    proprio_projector=proprio_projector if cfg.use_proprio else None,
                    noisy_action_projector=noisy_action_projector if cfg.use_diffusion else None,
                    action_head=action_head if (cfg.use_l1_regression or cfg.use_diffusion or cfg.use_rdt) else None,
