...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def get_last_hidden_states(
        ...
        **kwargs: str,
        ...
        """Get hidden states from the model for the given input sequence.
        ...
        Args:
            ...
            proprio_projector: Projector for proprioceptive features
            use_film: Whether to use FiLM conditioning
            **kwargs: Additional arguments including pixel_values, attention_mask, etc.

        Returns:
            text_hidden_states: Last layer hidden states for text tokens (excluding vision/proprio/action/stop)
            actions_hidden_states: Last layer hidden states for action tokens
        """
        # If the special empty token ('') does not already appear after the colon (':') token in the prompt
        # (after "OUT:" or "ASSISTANT:"), insert it to match the inputs seen at training time
        if not torch.all(input_ids[:, -1] == 29871):
        ...
        # Get number of tokens in prompt (excluding the start token)
        NUM_PROMPT_TOKENS = input_ids.shape[-1] - 1  # Mirror logic in `predict_action`
        ...
        # Count number of patches (include proprio if present)
        NUM_PATCHES = self.vision_backbone.get_num_patches() * self.vision_backbone.get_num_images_in_input()
        if use_proprio:
            NUM_PATCHES += 1
        ...
        # Slice out action hidden states (reuse indexing logic from `_regression_or_discrete_prediction`)
        actions_hidden_states = last_hidden_states[
            :,
            NUM_PATCHES + NUM_PROMPT_TOKENS : NUM_PATCHES + NUM_PROMPT_TOKENS + ACTION_DIM * NUM_ACTIONS_CHUNK,
            :,
        ...
        # Text hidden states (exclude BOS(0), patches, and action/stop tokens)
        text_hidden_states = last_hidden_states[
            :,
            NUM_PATCHES + 1 : NUM_PATCHES + NUM_PROMPT_TOKENS,
            :,
        ]

        return text_hidden_states, actions_hidden_states
