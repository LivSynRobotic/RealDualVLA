...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    # need to be synced so that any changes the user makes to the VLA modeling code will go into effect
...
def load_dataset_statistics(stats_file_path: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        # print(f"Successfully loaded dataset statistics from {stats_file_path}")
        return stats
        
    except json.JSONDecodeError as e:
        print(f"Error: Invalid JSON format in {stats_file_path}: {e}")
        return None
    except Exception as e:
        print(f"Error loading dataset statistics from {stats_file_path}: {e}")
        return None
...
def get_rdt_action_head(cfg: Any) -> torch.nn.Module:
    from fast_models.rdt_runner import RDTRunner
    print(f"Loading RDT action head via from_pretrained: {cfg.pretrained_rdt_path}")
    rdt_action_head = RDTRunner.from_pretrained(cfg.pretrained_rdt_path)
    rdt_action_head = rdt_action_head.to(torch.bfloat16).to(DEVICE)
    rdt_action_head.eval()
    return rdt_action_head
...
def _preprocess_images_for_siglip_224(pil_images: List[Image.Image]) -> torch.Tensor:
    transform = T.Compose([
        T.Resize(SIGLIP_IMAGE_SIZE, interpolation=T.InterpolationMode.BICUBIC),
        T.CenterCrop(SIGLIP_IMAGE_SIZE),
        T.ToTensor(),
        T.Normalize(mean=SIGLIP_MEAN, std=SIGLIP_STD),
    ])
    tensors = [transform(im) for im in pil_images]      # (3,224,224)
    stacked = torch.stack(tensors, dim=0)               # (N,3,224,224)
    pixel_values = stacked.view(1, stacked.shape[0]*3, SIGLIP_IMAGE_SIZE, SIGLIP_IMAGE_SIZE)
    return pixel_values.to(DEVICE, dtype=torch.bfloat16)


def prepare_rdt_image_inputs(
    observation: Dict[str, Any],
    cfg: Any
) -> torch.Tensor:
    images_list = [observation["full_image"]]
    
    if cfg.num_images_in_input > 1:
        print("Adding wrist images to images_list")
        images_list.extend([observation[k] for k in observation.keys() if "wrist" in k])
    ...
    for idx, img in enumerate(images_list):
        ...
        if isinstance(img, torch.Tensor):
            print(f"Image {idx} is a torch.Tensor, converting to numpy")
            img = img.detach().cpu().numpy()
        elif isinstance(img, Image.Image):
            print(f"Image {idx} is a PIL.Image, converting to numpy array")
            img = np.array(img)
    
        img = np.asarray(img)
    
        # 可能是 (3,H,W) -> 转成 (H,W,3)
        if img.ndim == 3 and img.shape[0] in (1, 3) and img.shape[-1] not in (1, 3) and img.shape[0] <= 3:
            print(f"Image {idx} is in CHW format, transposing to HWC")
            img = np.transpose(img, (1, 2, 0))
    
        # 去除多余 batch 维，如 (1,H,W,3)
        if img.ndim == 4 and img.shape[0] == 1:
            print(f"Image {idx} has an extra batch dimension, removing it")
            img = img[0]
    
        # 校验通道
        if img.ndim != 3 or img.shape[2] not in (1, 3):
            raise ValueError(f"图像索引 {idx} 形状异常: {img.shape}")
    
        # dtype 处理
        if img.dtype != np.uint8:
            print(f"Image {idx} dtype is {img.dtype}, converting to uint8")
            if np.issubdtype(img.dtype, np.floating):
                print(f"Image {idx} is a floating-point array, scaling to [0, 255]")
                img = np.clip(img * 255 if img.max() <= 1.0 else img, 0, 255)
            img = img.astype(np.uint8)
    
        # 单通道转 3 通道
        if img.shape[2] == 1:
            print(f"Image {idx} is single-channel, converting to 3-channel")
            img = np.repeat(img, 3, axis=2)
    
        pil_image = Image.fromarray(img, mode="RGB")
        pil_images.append(pil_image)
        print(f"Image {idx} processed and added to pil_images")

    # 移到循环外：预处理所有图片
    pixel_values = _preprocess_images_for_siglip_224(pil_images)

    # 保存处理后的图片用于验证（需要反标准化）
    for i, tensor in enumerate(pixel_values[0].view(-1, 3, SIGLIP_IMAGE_SIZE, SIGLIP_IMAGE_SIZE)):
        save_path = f"processed_pixel_value_{i}.png"
        
        # 反标准化：从 [-1, 1] 恢复到 [0, 1]
        # 反操作：denormalized = normalized * std + mean
        # 确保所有张量在同一设备上
        std_tensor = torch.tensor(SIGLIP_STD, device=tensor.device, dtype=tensor.dtype).view(3, 1, 1)
        mean_tensor = torch.tensor(SIGLIP_MEAN, device=tensor.device, dtype=tensor.dtype).view(3, 1, 1)
        denormalized_tensor = tensor * std_tensor + mean_tensor
        
        # 确保值在 [0, 1] 范围内
        denormalized_tensor = torch.clamp(denormalized_tensor, 0, 1)
        
        # 转换为 PIL 图像并保存 (convert to float32 for compatibility)
        T.ToPILImage()(denormalized_tensor.to(torch.float32).cpu()).save(save_path)
        print(f"Pixel value {i} saved to {save_path}")
...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        if action_head is None:
            ...
            # Custom action head for continuous actions (L1 regression or diffusion)
...
# 缓存 RDT 归一化统计
_RDT_NORM_STATS_CACHE: dict[str, dict] = {}

def _get_rdt_norm_stats(cfg: Any) -> Optional[Dict[str, Any]]:
    """优先从本地 cfg.pretrained_rdt_path 读取 dataset_statistics.json，并做缓存。"""
    try:
        ckpt_dir = getattr(cfg, "pretrained_rdt_path", None) or getattr(cfg, "pretrained_checkpoint", None)
        if not ckpt_dir:
            return None

        cache_key = f"rdt::{ckpt_dir}"
        if cache_key in _RDT_NORM_STATS_CACHE:
            return _RDT_NORM_STATS_CACHE[cache_key]
        ...
        if os.path.isfile(stats_path):
            ...
            try:
                if "/" in ckpt_dir and not os.path.isdir(ckpt_dir):
                    hub_path = hf_hub_download(repo_id=ckpt_dir, filename="dataset_statistics.json")
        ...
        if stats is not None:
            _RDT_NORM_STATS_CACHE[cache_key] = stats
        return stats
    except Exception:
        return None

def _unnormalize_actions_from_stats(
    actions: np.ndarray,
    ...
    if unnorm_key is None:
        ...
        if len(keys) != 1:
            raise ValueError("存在多个 norm_stats 键，请设置 cfg.unnorm_key。")
        unnorm_key = keys[0]
    ...
    if ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.BOUNDS:
        ...
        action_high, action_low = np.array(action_stats["max"]), np.array(action_stats["min"])
    elif ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.BOUNDS_Q99:
    ...
    return np.where(
        mask,
        0.5 * (actions + 1.0) * (action_high - action_low + 1e-8) + action_low,
        actions,
    )
...
def _get_rdt_action(
    ...
    siglip_vision_encoder: Optional[torch.nn.Module] = None,
) -> np.ndarray:
    from prismatic.vla.constants import ACTION_DIM
    DEVICE = text_hidden_states.device
    RGB_CHANNELS = 3
    CTRL_FREQ_DEFAULT = 10.0

    # Language tokens (already excludes action tokens / patches)
    lang_tokens = text_hidden_states
    batch_size = lang_tokens.shape[0]
    lang_attn_mask = torch.ones(batch_size, lang_tokens.shape[1], device=DEVICE, dtype=torch.bool)

    # Image tokens
    assert siglip_vision_encoder is not None, "SiglipVisionTower must be provided."
    with torch.no_grad():
        b, c, h, w = pixel_values.shape
        assert c % RGB_CHANNELS == 0, f"Expected the number of channels to be a multiple of 3 (RGB * N), but got {c}"
        num_images = c // RGB_CHANNELS
        # (B * N, 3, H, W)
        pixel_values = pixel_values.view(b * num_images, RGB_CHANNELS, h, w)
        img_tokens = siglip_vision_encoder(pixel_values).detach()
        img_tokens = img_tokens.reshape(b, num_images * img_tokens.shape[1], -1)

    # State tokens
    if cfg.use_proprio and proprio is not None:
        # 获取归一化统计信息并归一化 proprio
        print(f"Raw proprio before normalization: {proprio}")
        
        norm_stats = _get_rdt_norm_stats(cfg)
        if norm_stats is not None:
            ...
            normalized_proprio = _normalize_proprio_for_rdt(proprio, norm_stats[unnorm_key]["proprio"])
            state_tokens = torch.tensor(normalized_proprio, device=DEVICE, dtype=torch.bfloat16).unsqueeze(0).unsqueeze(0)
        else:
            print("WARNING: 无法获取归一化统计信息，使用原始 proprio 值")
            state_tokens = torch.tensor(proprio, device=DEVICE, dtype=torch.bfloat16).unsqueeze(0).unsqueeze(0)
    else:
        state_tokens = torch.zeros(batch_size, 1, ACTION_DIM, device=DEVICE, dtype=torch.bfloat16)

    # Action mask / ctrl freqs
    action_mask = torch.ones(batch_size, 1, ACTION_DIM, device=DEVICE, dtype=torch.bfloat16)
    ctrl_freqs = torch.full((batch_size,), CTRL_FREQ_DEFAULT, device=DEVICE)
    ...
    predicted_actions = action_head.predict_action(
        ...
        actions_hidden_states=actions_hidden_states.to(torch.bfloat16) if actions_hidden_states is not None else None,
    )
    predicted_actions_np = predicted_actions.float().cpu().detach().numpy()
    print(f"[INFO] RDT action head predicted actions: {predicted_actions_np}")

    try:
        norm_stats = _get_rdt_norm_stats(cfg)
        if norm_stats:
            unnorm_key = getattr(cfg, "unnorm_key", None)
            predicted_actions_np = _unnormalize_actions_from_stats(predicted_actions_np, norm_stats, unnorm_key)
    except Exception as e:
        raise RuntimeError(f"反归一化失败，程序退出: {e}")

    return predicted_actions_np

def _normalize_proprio_for_rdt(proprio: np.ndarray, proprio_stats: Dict[str, Any]) -> np.ndarray:
    if ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.BOUNDS:
        mask = proprio_stats.get("mask", np.ones_like(proprio_stats["min"], dtype=bool))
        proprio_high, proprio_low = np.array(proprio_stats["max"]), np.array(proprio_stats["min"])
    elif ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.BOUNDS_Q99:
        mask = proprio_stats.get("mask", np.ones_like(proprio_stats["q01"], dtype=bool))
        proprio_high, proprio_low = np.array(proprio_stats["q99"]), np.array(proprio_stats["q01"])
    else:
        raise ValueError("Unsupported action/proprio normalization type detected!")
    normalized_proprio = np.clip(
        np.where(
    ...
    zeros_mask = np.array(proprio_stats["min"]) == np.array(proprio_stats["max"])
    normalized_proprio = np.where(zeros_mask, 0.0, normalized_proprio)
